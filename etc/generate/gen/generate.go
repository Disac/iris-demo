...
func (g *Gen) genTable(v *Table, t *template.Template, pkg string, basePkg string) {
	...
	for _, c := range cols {
		...
		gc.Annotation = "`db:\"" + c.Field + "\" json:\"" + c.Field + "\"`"
		gc.DBName = c.Field
		if strings.Contains(colType, "char") || strings.Contains(colType, "text") {
		...
		} else if strings.Contains(colType, "bigint") {
			gc.Type = " int64"
			...
			gc.Type = " decimal.Decimal"
			needDecimalImport = true
		}
...
import (
	...
	{{if .needDecimalImport}}"github.com/shopspring/decimal"{{end}}
)
// {{.comment}}
type {{.name}} struct{
	{{range .cols}}
...
type {{.name}} struct{
	{{range .cols}}
	{{.Name}} {{if .CanNull}}*{{end}}{{.Type}} {{.Annotation}}// {{.Comment}}{{end}}
}
type {{.name}}Dao struct{
	...
	Table string
	{{if .useInterface}}
	DB itfc.SqlxHandle{{else}}DB *sqlx.DB{{end}}
}
{{if .useInterface}}
